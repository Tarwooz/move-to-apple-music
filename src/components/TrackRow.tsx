...
export default function TrackRow({ match, index, onSelectCandidate, onSkip, onManualSearch }: Props) {
  ...
  const [query, setQuery] = useState(`${stripBrackets(source.title)}-${source.artist}`);

  const handleSearch = async () => {
    if (!query.trim()) return;
    setSearching(true);
    await onManualSearch(match.id, query.trim());
    setSearching(false);
  };
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0">
              <div className="text-[13px] font-medium text-[#1D1D1F] truncate leading-snug">
                {selectedCandidate.trackName}
              </div>
              <div className="text-[11px] text-[#6E6E73] truncate mt-0.5">
                {selectedCandidate.artistName}
              </div>
            </div>
          </div>
        ) : (
